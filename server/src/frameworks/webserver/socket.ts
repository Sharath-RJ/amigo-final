    ...
    export default function configureSocket(server: HttpServer): SocketIOServer {
        ...
        io.on("connection", (socket) => {
            ...
            socket.on("joinRoom", (userId) => {
                ...
                console.log(
                    `User ${socket.id} with ID ${userId} joined the room`
                )
            })
            ...
            socket.on("sendMessage", (message) => {
                ...
                if (receiverSocket) {
                    ...
                    console.log(
                        `Message sent to user with ID: ${receiverSocket.id}.`
                    )
                } else {
        ...
        function emitNotificationCountUpdate(count: number) {
            io.emit("notificationCountUpdate", count)
        }

        
        (global as any).emitNotificationCountUpdate = emitNotificationCountUpdate;
